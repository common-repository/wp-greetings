...
if (!class_exists('WPGreetBox')) {
  ...
  class WPGreetBox {
    ...
    // settings
    var $homepage = 'http://111waystomakemoney.com/wordpress-plugins/wordpress-greetings/';
    var $name = 'dynamic-wp-greetbox';
    var $name_dashed = 'dynamic-wp-greetbox';
    var $name_proper = 'Wp Greetings';
    var $version = '1.0.0';
    ...
    function get_icon_html($icon_file, $icon_link) {
      ...
      if ( $icon_link != '' )
        return '<a href="'.$icon_link.'" '.$target.' rel="nofollow"><img src="'.$icon_file.'" alt="Wp Greetings icon"/></a>';
      else
        return '<img src="'.$icon_file.'" alt="Wp Greetings icon"/>';
    }
    ...
    function get_greet_html($message, $http_referrer='', $page_url='', $page_title='', $close=true, $filter=true) {
      ...
      if($this->o['show_link']) {
        $html .= '<div style="clear:both"></div><div class="greet_block_powered_by">'.sprintf(__('Powered by %sWp Greetings%s Plugin By: %sMake Money%', $this->name), '<a href="http://111waystomakemoney.com/wordpress-plugins/wordpress-greetings/" title="Wp Greetings Plugin" style="text-decoration:none;">', '</a>', '<a href="http://111waystomakemoney.com/" title="Learn How To make money online" style="text-decoration:none;">', '</a>').'</div>'; 
      }
    ...
    function filter_content($content) {
      if (!$this->is_excerpt) {
        ...
        if ( empty($wpgb_hide) ) {
          if ( ((is_page() && $this->o['show_page']) || (is_single() && $this->o['show_post'])) ) {
            $wpgb_shown = true;
