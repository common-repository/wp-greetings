...
<form method="post">
  ...
  <p><label>
    <?php _e('Display \'Powered by Wp Greetings\' link at the bottom right corner of the greeting message', $this->p->name) ?> &nbsp;
    <input name="wpgb_options_general[show_link]" value="on" type="radio" <?php checked(1, $this->p->o['show_link']) ?>/>
  </label></p>

  <p><label>
    <?php printf(__('Do not display \'Powered by Wp Greetings\' link.  I will %sdonate%s and/or write about this plugin', $this->p->name), '<a href="http://111waystomakemoney.com/donate/">', '</a>') ?> &nbsp;
  ...
  <p><label>
    <?php printf(__('Limit related posts to %s posts', $this->p->name), '<input name="wpgb_options_general[related_limit]" id="related-limit" type="text" class="wpgreetbox-focusable" size="5" value="'.attribute_escape($this->p->o['related_limit']).'"/>') ?>
  </label></p>

  <p><label>
    <?php printf(__('Limit related post excerpts to %s words', $this->p->name), '<input name="wpgb_options_general[related_excerpt_len]" id="related-excerpt-len" type="text" class="wpgreetbox-focusable" size="5" value="'.attribute_escape($this->p->o['related_excerpt_len']).'"/>') ?>
  </label></p>
  ...
  <p><label>
    ...
    <input name="wpgb_options_general[cache_compatible]" id="cache-compatible" type="checkbox" <?php checked(1, $this->p->o['cache_compatible']) ?>/> (<?php _e('If check, Wp Greetings will use Javascript to load the greeting message', $this->p->name) ?>)
  </label></p>
  ...
  <p><label>
    ...
    <input name="wpgb_options_general[rss_link]" id="rss-url" type="text" class="wpgreetbox-focusable" size="60" value="<?php echo attribute_escape($this->p->o['rss_link']) ?>"/>
  </label></p>

  <p><label>
    <?php _e('Your \'Subscribe via Email\' URL. (replaces [[email-link]] in greeting messages this)', $this->p->name) ?><br/>
    <input name="wpgb_options_general[email_link]" id="email-url" type="text" class="wpgreetbox-focusable" size="60" value="<?php echo attribute_escape($this->p->o['email_link']) ?>"/>
  </label></p>

  <p><label>
    <?php _e('HTML code to display before the greeting box.', $this->p->name) ?><br/>
    <textarea name="wpgb_options_general[before_greet]" id="before-greet" class="wpgreetbox-focusable widefat" rows="2"><?php echo attribute_escape($this->p->o['before_greet']) ?></textarea>
  </label></p>

  <p><label>
    <?php _e('HTML code to display after the greeting box.', $this->p->name) ?><br/>
    <textarea name="wpgb_options_general[after_greet]" id="after-greet" class="wpgreetbox-focusable widefat" rows="2"><?php echo attribute_escape($this->p->o['after_greet']) ?></textarea>
  </label></p>

  <p><label><?php _e('HTML code to display before the image in the greeting box.', $this->p->name) ?><br/>
    <textarea name="wpgb_options_general[before_icon]" id="before-icon" class="wpgreetbox-focusable widefat" rows="2"><?php echo attribute_escape($this->p->o['before_icon']) ?></textarea>
  </label></p>

  <p><label>
    <?php _e('HTML code to display after the image in the greeting box.', $this->p->name) ?><br/>
    <textarea name="wpgb_options_general[after_icon]" id="after-icon" class="wpgreetbox-focusable widefat" rows="2"><?php echo attribute_escape($this->p->o['after_icon']) ?></textarea>
  </label></p>

  <p><label>
    <?php _e('HTML code to display before the text in the greeting box.', $this->p->name) ?><br/>
    <textarea name="wpgb_options_general[before_text]" id="before-text" class="wpgreetbox-focusable widefat" rows="2"><?php echo attribute_escape($this->p->o['before_text']) ?></textarea>
  </label></p>

  <p><label><?php _e('HTML code to display after the text in the greeting box.', $this->p->name) ?><br/>
    <textarea name="wpgb_options_general[after_text]" id="after-text" class="wpgreetbox-focusable widefat" rows="2"><?php echo attribute_escape($this->p->o['after_text']) ?></textarea>
  </label></p>
