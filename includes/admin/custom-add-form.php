...
<div id="wpgb_add_<?php echo $status ?>" class="wpgb_add_form">
  <div id="wpgb_add_view_<?php echo $status ?>" class="wpgb_message_view" style="display:block">
    <form id="wpgb_add_form_<?php echo $status ?>" action="index.php" method="post"><?php _e('Referrer URL', $this->p->name) ?>:<br/>
      ...
      <input name="wpgb_message[icon]" type="text" class="wpgb_focusable wpgb_wide" value="<?php echo attribute_escape($m['icon']) ?>"/><br/><br/>
      <?php _e('Greeting Icon Link (optional)', $this->p->name)?>:<br/>
      <input name="wpgb_message[icon_link]" type="text" class="wpgb_focusable wpgb_wide" value="<?php echo attribute_escape($m['icon_link']) ?>"/><br/><br/>
      <?php _e('Greeting message', $this->p->name) ?>:<br/>
      <textarea name="wpgb_message[text]" class="wpgb_focusable wpgb_wide" rows="4"><?php echo attribute_escape($m['text']) ?></textarea><br/><br/>
